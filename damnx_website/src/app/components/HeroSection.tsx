...
const DamnxLanding = () => {
  ...
  // GSAP Hero Animations (dynamically imported on client)
  useEffect(() => {
    ...
    (async () => {
      try {
        // Dynamically import gsap and its TextPlugin so the server build
        // doesn't try to resolve these modules during SSR.
        const gsapModule = await import('gsap');
        const { TextPlugin } = await import('gsap/TextPlugin');
        const gsap = (gsapModule && (gsapModule as any).default) || gsapModule;
        ...
        // If GSAP isn't available in some environments, fail gracefully.
        // You can log the error during development if needed.
