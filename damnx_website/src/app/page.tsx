...
const DamnxLanding = () => {
  ...
  // Auto-expand navigation on mobile after 3 seconds
  useEffect(() => {
    const isMobile = window.innerWidth < 768;
    if (isMobile) {
      const timer = setTimeout(() => {
        setNavExpanded(true);
        setTimeout(() => setNavExpanded(false), 5000);
      }, 3000);
  ...
  return (
    <div className="bg-black text-white overflow-hidden relative">
      ...
      <div className="relative" style={{ zIndex: 2 }}>
        ...
        {/* Dynamic Island Navigation */}
                <nav 
          className={`fixed top-6 left-1/2 transform -translate-x-1/2 z-50 nav-island mx-auto ${navExpanded || menuOpen ? 'expanded' : 'collapsed'}`}
          onMouseEnter={() => !menuOpen && setNavExpanded(true)}
          onMouseLeave={() => !menuOpen && setNavExpanded(false)}
        >
                  <div className="px-6 sm:px-7 py-3 sm:py-3.5">
            <div className={`flex items-center relative z-10 ${navExpanded || menuOpen ? 'justify-between' : 'justify-center'}`}>

         
              {/* Logo - Always Visible */}
              <div className="nav-text-container flex-shrink-0">

                <div className="nav-text-content text-xl sm:text-2xl font-bold tracking-tight nav-logo whitespace-nowrap">
                  <span className="text-red-500/90">DAMNX</span>
                  <span className="text-white/80 font-light"> Solutions</span>
                </div>
              </div>
              
              {/* Desktop Menu - Visible when expanded */}
              <div className={`hidden md:flex items-center space-x-8 nav-content ${navExpanded ? 'visible' : ''}`}>
                <a href="#services" className="text-gray-300 hover:text-red-500 transition-colors font-medium text-sm whitespace-nowrap">Services</a>
                <a href="#about" className="text-gray-300 hover:text-red-500 transition-colors font-medium text-sm whitespace-nowrap">About</a>
                <a href="#contact" className="text-gray-300 hover:text-red-500 transition-colors font-medium text-sm whitespace-nowrap">Contact</a>
                <button className="px-5 py-2.5 bg-red-500 hover:bg-red-600 rounded-full font-semibold transition-all text-sm whitespace-nowrap">
                  Get Started
                </button>
              </div>
            ...
            <div className={`md:hidden mt-4 pt-4 border-t border-red-900/30 nav-content ${menuOpen ? 'visible' : ''}`}>
              <div className="flex flex-col space-y-3">
                <a href="#services" className="text-gray-300 hover:text-red-500 transition-colors font-medium py-2">Services</a>
                                <a href="#about" className="text-gray-300 hover:text-red-500 transition-colors font-medium py-2">About</a>
                <a href="#contact" className="text-gray-300 hover:text-red-500 transition-colors font-medium py-2">Contact</a>
                <button className="px-5 py-2.5 bg-gradient-to-r from-red-600 to-red-500 rounded-full font-semibold text-sm mt-2">
                  Get Started
                </button>
